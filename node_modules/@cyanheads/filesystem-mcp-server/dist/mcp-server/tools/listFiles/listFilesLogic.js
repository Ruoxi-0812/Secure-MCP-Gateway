 ...
 */
const readDirectoryRecursive = async (dirPath, includeNested, context, state // Pass state object
) => {
    ...
    for (const entry of entries) {
        ...
        if (item.isDirectory && includeNested) {
            ...
            try {
                ...
                logger.error(`Error reading nested directory ${itemPath}`, { ...context, error: recursiveError.message, code: recursiveError.code });
                // Log the error and mark the item
                const errorMessage = recursiveError?.message || recursiveError?.message || 'Unknown error reading directory';
                logger.error(`Error reading nested directory ${itemPath}`, { ...context, error: errorMessage, code: recursiveError?.code });
 ...
 */
const formatTree = (items, truncated, prefix = '') => {
    let treeString = '';
 ...
 */
export const listFilesLogic = async (input, context) => {
