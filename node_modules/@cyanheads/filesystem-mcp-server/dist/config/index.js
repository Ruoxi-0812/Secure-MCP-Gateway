...
let resolvedFsBaseDirectory = env.FS_BASE_DIRECTORY;
if (env.FS_BASE_DIRECTORY && !path.isAbsolute(env.FS_BASE_DIRECTORY)) {
    ...
    if (process.stdout.isTTY) {
        console.log(`Info: Relative FS_BASE_DIRECTORY "${env.FS_BASE_DIRECTORY}" resolved to "${resolvedFsBaseDirectory}".`);
    }
}
if (process.stdout.isTTY) {
    if (resolvedFsBaseDirectory) {
        ...
        try {
            ...
            if (resolvedFsBaseDirectory) {
                console.log(`Info: Filesystem operations will be restricted to base directory: ${resolvedFsBaseDirectory}`);
            }
        }
        catch (error) {
        ...
        console.warn("Warning: FS_BASE_DIRECTORY is not set. Filesystem operations will not be restricted to a base directory. This is a potential security risk.");
    }
}
